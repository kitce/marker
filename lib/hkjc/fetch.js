...
module.exports = async () => {
  ...
  const fetch = async (date) => {
    ...
    if (data) {
      ...
      console.log(`${_markSix.date}, saved ${_markSix.number}`);
      return _markSix;
    } else {
...
/**
 * Get the list of fetch dates by reading the records directory
 * @private
 * @return {Promise} it resolves <Array<Moment>>
 */
async function getFetchedDates () {
  const filenames = await readdirAsync(config.recordsDir);
  return _.map(filenames, (filename) => {
    const date = _.split(filename, '.')[0];
    return moment(date, config.standardDateFormat);
  });
}
