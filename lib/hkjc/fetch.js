...
module.exports = async () => {
  ...
  const fetch = async (date) => {
    ...
    if (!data) {
      ...
      const _markSix = await MarkSix.init(data).save();
      console.log(`${_markSix.date}, saved ${_markSix.number} to ${_markSix.filename}`);
      markSixes.push(_markSix);
    }
  };
  for (let i = 0; i < missingDates.length; i++) {
    const missingDate = missingDates[i];
    await fetch(missingDate);
  }
  return markSixes;
};
